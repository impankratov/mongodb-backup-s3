...
echo "=> Backup started"
if mongodump --host ${MONGODB_HOST} ${USER_STR}${PASS_STR}${DB_STR} --archive=\${BACKUP_NAME} --gzip ${EXTRA_OPTS} && aws s3 --region ${BUCKET_REGION} cp \${BACKUP_NAME} \${S3BACKUP} && aws s3 cp --region ${BUCKET_REGION} \${S3BACKUP} \${S3LATEST} && rm \${BACKUP_NAME} ;then
...
if aws s3 cp --region ${BUCKET_REGION} \${S3RESTORE} \${RESTORE_ME} && mongorestore --host ${MONGODB_HOST} ${USER_STR}${PASS_STR}${DB_STR} --drop --archive=\${RESTORE_ME} --gzip && rm \${RESTORE_ME}; then
...
#!/bin/bash
aws s3 --region ${BUCKET_REGION} ls ${S3PATH}
EOF
...
if [ -z "${DISABLE_CRON}" ]; then
    ...
    crond -l 2 && tail -f /mongo_backup.log
fi
